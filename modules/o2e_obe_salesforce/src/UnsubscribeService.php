...
/**
 * Unsubscribe Service class is unsubscribe user form saleforce.
 */
class UnsubscribeService {
   ...
   * Constructor method.
   */
  public function __construct(Client $http_client, LoggerChannelFactory $logger_factory, State $state, AuthTokenManager $auth_token_manager) {
  ...
  public function unsubscribe(array $options = []) {
    ...
    $api_url = $this->authTokenManager->getSfConfig('unsubscribe.api_url_segment');
    ...
    try {
      ...
      $this->loggerFactory->get('Salesforce - Unsubscribe')->notice(UrlHelper::buildQuery($options) . ' ' . $response->getStatusCode());
      return $response->getStatusCode();
    }
    catch (RequestException $e) {
      $this->loggerFactory->get('Salesforce - Unsubscribe Fail')->error($e->getMessage());
    }
  }

...
class CreateLead {
  ...
  public function __construct(Client $http_client, ObeSfLogger $obe_sf_logger, State $state, 
    ...
    TimeInterface $time_service, RequestStack $request_stack, ConfigFactoryInterface $config, 
  ...
  public function create_lead(array $options = []) {
    ...
    try {
      ...
      $duration = round($endCreateLeadTimer - $startCreateLeadTimer, 2);
      ...
      // Datadog
      $this->dataDogService->createSuccessDatadog('Salesforce - Create Lead', 'POST', $api_url, $response, $duration); 
